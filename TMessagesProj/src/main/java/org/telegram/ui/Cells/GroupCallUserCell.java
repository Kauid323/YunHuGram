...
public class GroupCallUserCell extends FrameLayout {
        ...
        if (id > 0) {
            ...
            if (currentUser != null && currentUser.verified) {
                ...
                if (premiumDrawable == null) {
                    premiumDrawable = getContext().getResources().getDrawable(R.drawable.msg_settings).mutate();
    ...
    private void applyParticipantChanges(boolean animated, boolean internal) {
        ...
        if (isSelfUser()) {
            newMuted = CallService.getSharedInstance() != null && CallService.getSharedInstance().isMicMute() && (!isSpeaking || !hasVoice);
        } else {

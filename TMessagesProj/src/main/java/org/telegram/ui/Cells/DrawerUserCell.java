...
public class DrawerUserCell extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == NotificationCenter.currentUserPremiumStatusChanged) {
            if (account == accountNumber) {
                setAccount(accountNumber, yhUser);
            }
    ...
    public void setAccount(int account) {
        setAccount(account, yhUser);
    }

    public void setAccount(int account, YhSelfUser yhUser) {
        accountNumber = account;
        this.yhUser = yhUser;
        TLRPC.User user = UserConfig.getInstance(accountNumber).getCurrentUser();
        if (user == null) {
            bindYhAccount(accountNumber, yhUser);
            return;
        }
    ...
    private void bindYhAccount(int account, YhSelfUser yhUser) {
        YhSession session = YhApiFacade.getInstance(account).getSession();
        if ((yhUser == null || TextUtils.isEmpty(yhUser.getName())) && (session == null || !session.isValid())) {
            textView.setText("");
            status.set((Drawable) null, true);
            textView.setRightDrawableOutside(false);
            imageView.getImageReceiver().setImageBitmap((Drawable) null);
            checkBox.setVisibility(account == UserConfig.selectedAccount ? VISIBLE : INVISIBLE);
            return;
        }

        String title = yhUser != null && !TextUtils.isEmpty(yhUser.getName()) ? yhUser.getName() : "云湖账号";
        ...
        status.set((Drawable) null, true);
        textView.setRightDrawableOutside(false);
        textView.setDrawablePadding(0);

        int avatarId = 0;
        if (yhUser != null && !TextUtils.isEmpty(yhUser.getId())) {
            avatarId = yhUser.getId().hashCode();
        } else if (session != null && !TextUtils.isEmpty(session.getUserId())) {
            avatarId = session.getUserId().hashCode();
        }
        avatarDrawable.setInfo(avatarId, title, null);
        imageView.getImageReceiver().setCurrentAccount(account);
        if (yhUser != null && !TextUtils.isEmpty(yhUser.getAvatarUrl())) {
            imageView.setImage(YhImageUrlHelper.getAvatarThumbUrl(yhUser.getAvatarUrl()), "50_50", avatarDrawable);
        } else {
            imageView.getImageReceiver().setImageBitmap(avatarDrawable);
        }
        checkBox.setVisibility(account == UserConfig.selectedAccount ? VISIBLE : INVISIBLE);
    }

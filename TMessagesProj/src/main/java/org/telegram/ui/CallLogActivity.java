	...
	public void didReceivedNotification(int id, int account, Object... args) {
		if (id == NotificationCenter.didReceiveNewMessages) {
			...
			if (chatFull.id == waitingForCallChatId) {
				...
				if (groupCall != null) {
                    CallHelper.startCall(lastCallChat, null, null, false, getParentActivity(), CallLogActivity.this, getAccountInstance());
	...
	private class CallCell extends FrameLayout {
		...
		public CallCell(Context context) {
			...
			imageView.setOnClickListener(v -> {
                ...
                CallHelper.startCall(lastCallUser = row.user, row.video, row.video || userFull != null && userFull.video_calls_available, getParentActivity(), null, getAccountInstance());
			});
	...
	public View createView(Context context) {
		...
		floatingButton.setOnClickListener(v -> {
			...
			contactsFragment.setDelegate((user, param, activity) -> {
                ...
                CallHelper.startCall(lastCallUser = user, false, userFull != null && userFull.video_calls_available, getParentActivity(), null, getAccountInstance());
			});
		...
		if (requestCode == 101 || requestCode == 102 || requestCode == 103) {
			...
			if (grantResults.length > 0 && allGranted) {
				if (requestCode == 103) {
                    ...
                    CallHelper.startCall(lastCallUser, requestCode == 102, requestCode == 102 || userFull != null && userFull.video_calls_available, getParentActivity(), null, getAccountInstance());
				}
			} else {

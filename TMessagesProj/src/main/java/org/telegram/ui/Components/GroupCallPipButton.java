...
public class GroupCallPipButton extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, CallService.StateListener {
    ...
    public void setState(int state) {
        ...
        if (callService != null && ChatObject.isChannelOrGiga(callService.getChat())) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (!stub) {
            ...
            boolean isMuted = CallService.getSharedInstance() != null && CallService.getSharedInstance().isMicMute();
    ...
    private void updateButtonState() {
        ...
        if (callService != null && callService.groupCall != null) {
            int currentCallState = callService.getCallState();
            if (currentCallState == CallService.STATE_WAIT_INIT || currentCallState == CallService.STATE_WAIT_INIT_ACK || currentCallState == CallService.STATE_CREATING || currentCallState == CallService.STATE_RECONNECTING) {
                ...
                TLRPC.TL_groupCallParticipant participant = callService.groupCall.participants.get(callService.getSelfId());
                if (participant != null && !participant.can_self_unmute && participant.muted && !ChatObject.canManageCalls(callService.getChat())) {
                    if (!callService.isMicMute()) {
                        callService.setMicMute(true, false, false);
                    }
                    ...
                    boolean isMuted = callService.isMicMute();

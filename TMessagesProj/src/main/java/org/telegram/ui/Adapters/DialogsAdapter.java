...
public class DialogsAdapter extends RecyclerListView.SelectionAdapter implements DialogCell.DialogCellDelegate {
    ...
    private boolean yhDialogsEnabled;
    private boolean yhDialogsLoading;
    private ArrayList<YhDialogItem> yhDialogs = new ArrayList<>();
    ...
    public void setYhDialogs(ArrayList<YhDialogItem> dialogs) {
        yhDialogs.clear();
        if (dialogs != null) {
            yhDialogs.addAll(dialogs);
        }
    }

    public void setYhDialogsEnabled(boolean enabled) {
        yhDialogsEnabled = enabled;
    }

    public void setYhDialogsLoading(boolean loading) {
        yhDialogsLoading = loading;
    }

    public YhDialogItem getYhDialogItem(int position) {
        if (position < 0 || position >= itemInternals.size()) {
            return null;
        }
        return itemInternals.get(position).yhDialog;
    }
...
//        }
    ...
    private class ItemInternal extends AdapterWithDiffUtils.Item {
        ...
        public ItemInternal(int viewType, YhDialogItem yhDialog) {
            super(viewType, true);
            this.yhDialog = yhDialog;
            if (yhDialog != null) {
                long stableKey = yhDialog.getStableKey();
                int currentId = dialogsStableIds.get(stableKey, -1);
                if (currentId >= 0) {
                    stableId = currentId;
                } else {
                    stableId = stableIdPointer++;
                    dialogsStableIds.put(stableKey, stableId);
                }
            } else {
                stableId = stableIdPointer++;
            }
        ...
        boolean compare(ItemInternal itemInternal) {
            ...
            if (viewType == VIEW_TYPE_DIALOG) {
                if (yhDialog != null || itemInternal.yhDialog != null) {
                    return yhDialog != null && itemInternal.yhDialog != null
                            && Objects.equals(yhDialog.getChatId(), itemInternal.yhDialog.getChatId())
                            && yhDialog.getChatType() == itemInternal.yhDialog.getChatType();
                }
        ...
        @Override
        public int hashCode() {
            return Objects.hash(dialog, recentMeUrl, contact, yhDialog);
        }
    }
    ...
    public TLObject getItem(int i) {
        ...
        } else if (itemInternals.get(i).yhDialog != null) {
            return null;
        }
        return null;
    }
    ...
    public void updateHasHints() {
        hasHints = !yhDialogsEnabled && folderId == 0 && dialogsType == DialogsActivity.DIALOGS_TYPE_DEFAULT && !isOnlySelect && !MessagesController.getInstance(currentAccount).hintDialogs.isEmpty();
    }
...
//                firstUpdate = false;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int i) {
        switch (holder.getItemViewType()) {
            case VIEW_TYPE_DIALOG: {
                YhDialogItem yhDialog = getYhDialogItem(i);
                YhDialogItem nextYhDialog = getYhDialogItem(i + 1);
                TLRPC.Dialog dialog = yhDialog == null ? (TLRPC.Dialog) getItem(i) : null;
                TLRPC.Dialog nextDialog = nextYhDialog == null ? (TLRPC.Dialog) getItem(i + 1) : null;
                ...
                } else if (yhDialog != null) {
                    DialogCell cell = (DialogCell) holder.itemView;
                    cell.useSeparator = nextYhDialog != null;
                    cell.fullSeparator = false;
                    cell.setChecked(false, false);
                    cell.setDialog(createCustomDialog(yhDialog));
                    cell.checkHeight();
    ...
    private DialogCell.CustomDialog createCustomDialog(YhDialogItem dialog) {
        DialogCell.CustomDialog customDialog = new DialogCell.CustomDialog();
        customDialog.id = dialog.getDisplayId();
        customDialog.name = dialog.getTitle() == null ? "" : dialog.getTitle();
        customDialog.message = dialog.getPreviewText() == null ? "" : dialog.getPreviewText();
        customDialog.unread_count = dialog.isUnread() ? 1 : 0;
        customDialog.pinned = false;
        customDialog.muted = dialog.isMuted();
        customDialog.type = 0;
        customDialog.date = (int) Math.min(Integer.MAX_VALUE, Math.max(0L, dialog.getTimestampSeconds()));
        customDialog.verified = dialog.isVerified();
        customDialog.isMedia = false;
        customDialog.avatarUrl = YhImageUrlHelper.getDetailImageUrl(dialog.getAvatarUrl());
        return customDialog;
    }
    ...
    private void updateItemList() {
        ...
        if (yhDialogsEnabled) {
            dialogsCount = yhDialogs.size();
            isEmpty = false;
            onlineContacts = null;
            hasChatlistHint = false;

            if (yhDialogsLoading && dialogsCount == 0) {
                return;
            }

            if (collapsedView || isTransitionSupport) {
                for (int i = 0; i < yhDialogs.size(); i++) {
                    itemInternals.add(new ItemInternal(VIEW_TYPE_DIALOG, yhDialogs.get(i)));
                }

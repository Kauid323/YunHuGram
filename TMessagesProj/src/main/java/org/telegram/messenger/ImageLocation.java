...
public class ImageLocation {
    ...
    public static ImageLocation getForDocument(TLRPC.Document document) {
        ...
        YhMediaRegistry.MediaEntry yhEntry = isYhSyntheticDcId(document.dc_id) ? YhMediaRegistry.findDocument(document.id) : null;
        if (yhEntry != null && yhEntry.url != null) {
            return getForRemotePath(yhEntry.url, yhEntry.size > 0 ? yhEntry.size : document.size);
        }
        ImageLocation imageLocation = new ImageLocation();
    ...
    public static ImageLocation getForPhoto(TLRPC.PhotoSize photoSize, TLRPC.Photo photo) {
        ...
        YhMediaRegistry.MediaEntry yhEntry = isYhSyntheticDcId(photo.dc_id) ? YhMediaRegistry.findPhoto(photo.id) : null;
        if (yhEntry != null && yhEntry.url != null) {
            return getForRemotePath(yhEntry.url, yhEntry.size > 0 ? yhEntry.size : photoSize.size);
        }
        int dc_id;
        if (photo.dc_id != 0) {
            dc_id = photo.dc_id;
    ...
    public static ImageLocation getForDocument(TLRPC.PhotoSize photoSize, TLRPC.Document document) {
        ...
        YhMediaRegistry.MediaEntry yhEntry = isYhSyntheticDcId(document.dc_id) ? YhMediaRegistry.findDocument(document.id) : null;
        if (yhEntry != null) {
            String previewUrl = yhEntry.previewUrl != null ? yhEntry.previewUrl : yhEntry.url;
            if (previewUrl != null) {
                return getForRemotePath(previewUrl, photoSize.size);
            }
        }
        return getForPhoto(photoSize.location, photoSize.size, null, document, null, TYPE_SMALL, document.dc_id, null, photoSize.type);
    }

    private static ImageLocation getForRemotePath(String path, long size) {
        ImageLocation imageLocation = getForPath(path);
        if (imageLocation != null) {
            imageLocation.currentSize = Math.max(size, 0L);
        }
        return imageLocation;
    }

    private static boolean isYhSyntheticDcId(int dcId) {
        return dcId == YhMediaRegistry.YH_SYNTHETIC_DC_ID;
    }

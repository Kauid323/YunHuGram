...
public class ImageReceiver implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void setForUserOrChat(TLObject object, Drawable avatarDrawable, Object parentObject, boolean animationEnabled, int vectorType, boolean big) {
        ...
        if (vectorImageMarkup != null && vectorType != 0) {
            ...
            long yhPeerId = 0;
            if (object instanceof TLRPC.User) {
                yhPeerId = ((TLRPC.User) object).id;
            } else if (object instanceof TLRPC.Chat) {
                yhPeerId = ((TLRPC.Chat) object).id;
            }
            String yhAvatarUrl = yhPeerId != 0 ? YhPeerRegistry.findAvatarUrl(yhPeerId) : null;
            ...
            } else if (location == null && yhAvatarUrl != null) {
                String resolvedAvatarUrl = big ? YhImageUrlHelper.getAvatarDetailUrl(yhAvatarUrl) : YhImageUrlHelper.getAvatarThumbUrl(yhAvatarUrl);
                ImageLocation yhLocation = ImageLocation.getForPath(resolvedAvatarUrl);
                if (strippedBitmap != null) {
                    setImage(yhLocation, filter, strippedBitmap, null, parentObject, 0);
                } else if (hasStripped) {
                    setImage(yhLocation, filter, ImageLocation.getForUserOrChat(object, ImageLocation.TYPE_STRIPPED), "50_50_b", avatarDrawable, parentObject, 0);
                } else {
                    setImage(yhLocation, filter, avatarDrawable, null, parentObject, 0);
                }
            } else {
                if (strippedBitmap != null) {

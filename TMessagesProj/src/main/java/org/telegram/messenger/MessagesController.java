...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public MessagesController(int num) {
        ...
        premiumLocked = BuildVars.PREMIUM_DISABLED ? false : mainPreferences.getBoolean("premiumLocked", false);
        ...
        for (int a = 0, N = object.value.size(); a < N; a++) {
            ...
            switch (value.key) {
                ...
                case "premium_purchase_blocked": {
                    if (value.value instanceof TLRPC.TL_jsonBool) {
                        boolean newPremiumLocked = BuildVars.PREMIUM_DISABLED ? false : ((TLRPC.TL_jsonBool) value.value).value;
                        if (premiumLocked != newPremiumLocked) {
                            premiumLocked = newPremiumLocked;
...
//                            if (newMsg == null && oldMs.getId() > 0 || newMsg != null && newMsg.messageOwner.date > oldMsg.messageOwner.date)
    ...
    public void addUsersToChat(TLRPC.Chat currentChat, BaseFragment baseFragment, ArrayList<TLRPC.User> users, int fwdCount, Consumer<TLRPC.User> onAddUser, Consumer<TLRPC.User> onRestricted, Runnable onComplete) {
        ...
        final Runnable showUserRestrictedPrivacyAlert = () -> {
            AndroidUtilities.runOnUIThread(() -> { }, 200);
        };
            ...
            if (updatesOnMainThreadFinal != null) {
                ...
                for (int a = 0, size = updatesOnMainThreadFinal.size(); a < size; a++) {
                    ...
                    if (baseUpdate instanceof TLRPC.TL_updatePrivacy) {
                            ...
                            if (chat instanceof TLRPC.TL_channelForbidden || chat.kicked) {
                                ...
                                if (call != null) {
                                    ...
                                    if (CallService.getSharedInstance() != null) {
                                        CallService.getSharedInstance().onGroupCallUpdated(updateGroupCall.call);
                                    }
                                }
                            }
                        }
                        ...
                        CallService svc = CallService.getSharedInstance();
                        if (svc != null) {
                        ...
                        if (CallService.getSharedInstance() != null) {
                            CallService.getSharedInstance().onGroupCallParticipantsUpdate(update);
                        }
                    } else if (baseUpdate instanceof TLRPC.TL_updateGroupCall) {
                        ...
                        if (CallService.getSharedInstance() != null) {
                            CallService.getSharedInstance().onGroupCallUpdated(update.call);
                        }
                    } else if (baseUpdate instanceof TLRPC.TL_updatePhoneCall) {
                        ...
                        if (call instanceof TLRPC.TL_phoneCallRequested) {
                            ...
                            if (svc != null || CallService.callIShouldHavePutIntoIntent != null || !callStateIsIdle) {
                                if (BuildVars.LOGS_ENABLED) {
                            ...
                            if (BuildVars.LOGS_ENABLED) {
                                FileLog.d("Auto-declining call " + call.id + " because VoIP is disabled");
                            }
                            TLRPC.TL_phone_discardCall req = new TLRPC.TL_phone_discardCall();
                            req.peer = new TLRPC.TL_inputPhoneCall();
                            req.peer.access_hash = call.access_hash;
                            ...
                            } else if (CallService.callIShouldHavePutIntoIntent != null) {
                                if (BuildVars.LOGS_ENABLED) {
                                    FileLog.d("Updated the call while the service is starting");
                                }
                                if (call.id == CallService.callIShouldHavePutIntoIntent.id) {
                                    CallService.callIShouldHavePutIntoIntent = call;
                                }
                            }
                        }
                    } else if (baseUpdate instanceof TLRPC.TL_updateDialogUnreadMark) {
    ...
    public boolean storiesEnabled() {
        if (BuildVars.PREMIUM_DISABLED) {
            return !"disabled".equals(storiesPosting);
        }
    ...
    public boolean storyEntitiesAllowed() {
        if (BuildVars.PREMIUM_DISABLED) {
            return !"disabled".equals(storiesEntities);
        }

...
//    private final FileLoaderPriorityQueue audioQueue = new FileLoaderPriorityQueue("audioQueue queue", 3);
    ...
    public void cancelLoadFile(TLRPC.Document document) {
        String yhUrl = getYhDocumentUrl(document);
        if (!TextUtils.isEmpty(yhUrl)) {
            ImageLoader.getInstance().cancelLoadHttpFile(yhUrl);
            return;
        }
...
//            }
    ...
    public boolean isLoadingFile(final String fileName) {
        return fileName != null && (loadOperationPathsUI.containsKey(fileName) || ImageLoader.getInstance().isLoadingHttpFileByKey(fileName));
    }
    ...
    public void loadFile(TLRPC.Document document, Object parentObject, int priority, int cacheType) {
        ...
        String yhUrl = getYhDocumentUrl(document);
        if (!TextUtils.isEmpty(yhUrl)) {
            ImageLoader.getInstance().loadHttpFile(yhUrl, getMimeTypePart(document.mime_type), currentAccount, getAttachFileName(document));
            return;
        }
    ...
    public File getPathToAttach(TLObject attach, String size, String ext, boolean forceCache, boolean useFileDatabaseQueue) {
        ...
        if (forceCache) {
            ...
            if (attach instanceof TLRPC.Document) {
                ...
                String yhUrl = getYhDocumentUrl(document);
                if (!TextUtils.isEmpty(yhUrl)) {
                    return ImageLoader.getHttpFilePath(yhUrl, getMimeTypePart(document.mime_type));
                }
                ...
                String yhPhotoUrl = getYhPhotoUrl(photoSize);
                if (!TextUtils.isEmpty(yhPhotoUrl)) {
                    return ImageLoader.getHttpFilePath(yhPhotoUrl, ext != null ? ext : "jpg");
                }
    ...
    public static String getAttachFileName(TLObject attach, String size, String ext) {
        if (attach instanceof TLRPC.Document) {
            ...
            String yhUrl = getYhDocumentUrl(document);
            if (!TextUtils.isEmpty(yhUrl)) {
                return Utilities.MD5(yhUrl) + "." + ImageLoader.getHttpUrlExtension(yhUrl, getMimeTypePart(document.mime_type));
            }
            String docExt;
            ...
            String yhPhotoUrl = getYhPhotoUrl(photo);
            if (!TextUtils.isEmpty(yhPhotoUrl)) {
                return Utilities.MD5(yhPhotoUrl) + "." + ImageLoader.getHttpUrlExtension(yhPhotoUrl, ext != null ? ext : "jpg");
            }
    ...
    private static String getYhDocumentUrl(TLRPC.Document document) {
        if (document == null) {
            return null;
        }
        YhMediaRegistry.MediaEntry entry = YhMediaRegistry.findDocument(document.id);
        return entry == null ? null : entry.url;
    }

    private static String getYhPhotoUrl(TLRPC.PhotoSize photoSize) {
        if (photoSize == null || photoSize.location == null) {
            return null;
        }
        YhMediaRegistry.MediaEntry entry = YhMediaRegistry.findPhoto(photoSize.location.volume_id);
        return entry == null ? null : entry.url;
    }

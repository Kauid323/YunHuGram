...
import org.telegram.yh.contacts.YhContactsRepository;
import org.telegram.yh.contacts.YhContactsViewModel;
...
public class ContactsController extends BaseController {
    ...
    public HashMap<String, ArrayList<Long>> yhMixedSectionsDict = new HashMap<>();
    public ArrayList<String> yhSortedMixedSectionsArray = new ArrayList<>();
    ...
    private final YhContactsViewModel yhContactsViewModel;
    ...
    public ContactsController(int instance) {
        super(instance);
        yhContactsViewModel = new YhContactsViewModel(instance);
    ...
    public void cleanup() {
        ...
        yhMixedSectionsDict.clear();
        yhSortedMixedSectionsArray.clear();
    ...
    public void loadContacts(boolean fromCache, final long hash) {
        ...
        if (shouldUseYhContacts()) {
            loadYhContacts(fromCache);
            return;
        }
        if (fromCache) {
            if (BuildVars.LOGS_ENABLED) {
    ...
    private void loadYhContacts(boolean fromCache) {
        boolean force = !fromCache;
        yhContactsViewModel.loadAddressBook(force, new YhContactsRepository.Callback() {
            @Override
            public void onSuccess(ArrayList<YhAddressBookEntry> entries) {
                ArrayList<TLRPC.TL_contact> contactsArr = new ArrayList<>();
                ArrayList<TLRPC.User> usersArr = new ArrayList<>();
                LongSparseArray<Boolean> existing = new LongSparseArray<>();
                HashMap<String, ArrayList<Long>> mixedSectionsDict = new HashMap<>();
                ArrayList<String> sortedMixedSectionsArray = new ArrayList<>();
                Collator collator = getLocaleCollator();
                if (entries != null) {
                    for (int i = 0; i < entries.size(); i++) {
                        YhAddressBookEntry entry = entries.get(i);
                        if (entry == null || TextUtils.isEmpty(entry.getChatId())) {
                            continue;
                        }
                        int chatType = entry.getChatType();
                        long peerId = YhTelegramMapper.syntheticPeerId(entry.getChatId(), chatType);
                        YhPeerRegistry.registerPeer(peerId, entry.getChatId(), chatType);
                        YhPeerRegistry.registerAvatarUrl(peerId, YhImageUrlHelper.getAvatarDetailUrl(entry.getAvatarUrl()));
                        if (chatType == YhChatType.GROUP) {
                            TLRPC.TL_chat chat = new TLRPC.TL_chat();
                            chat.id = peerId;
                            chat.title = TextUtils.isEmpty(entry.getName()) ? "群聊" : entry.getName();
                            ...
                            putYhMixedSectionItem(mixedSectionsDict, sortedMixedSectionsArray, entry.getName(), peerId);
                            continue;
                        }
                        if (existing.get(peerId) != null) {
                            continue;
                        }
                        existing.put(peerId, true);
                        TLRPC.TL_user user = new TLRPC.TL_user();
                        user.id = peerId;
                        user.first_name = TextUtils.isEmpty(entry.getName()) ? "云湖用户" : entry.getName();
                        user.last_name = "";
                        user.bot = chatType == YhChatType.BOT;
                        user.contact = true;
                        ...
                        TLRPC.TL_contact contact = new TLRPC.TL_contact();
                        contact.user_id = peerId;
                        contact.mutual = true;
                        contactsArr.add(contact);
                        putYhMixedSectionItem(mixedSectionsDict, sortedMixedSectionsArray, entry.getName(), peerId);
                    }
                }
                Collections.sort(sortedMixedSectionsArray, (left, right) -> {
                ...
                AndroidUtilities.runOnUIThread(() -> {
                    yhMixedSectionsDict = mixedSectionsDict;
                    yhSortedMixedSectionsArray = sortedMixedSectionsArray;
                });
                processLoadedContacts(contactsArr, usersArr, 0);
            }

            @Override
            public void onError(String error) {
                processLoadedContacts(new ArrayList<>(contacts), new ArrayList<>(), 2);
            }
        });
    }

    private void putYhMixedSectionItem(HashMap<String, ArrayList<Long>> sectionsDict, ArrayList<String> sortedSectionsArray,
            String name, long peerId) {
        ...
        ArrayList<Long> sectionItems = sectionsDict.get(key);
        if (sectionItems == null) {
            sectionItems = new ArrayList<>();
            sectionsDict.put(key, sectionItems);
            sortedSectionsArray.add(key);
        }
        sectionItems.add(peerId);
    }

    public boolean hasYhMixedContacts() {
        return !yhSortedMixedSectionsArray.isEmpty();
    }
